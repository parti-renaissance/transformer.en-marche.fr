...
import { toggleThemeFacet, resetParams, THEME } from '../actions/search-actions'
import '../../scss/dropdowns.css'
import './../../scss/theme.css'

function filterMeasuresForState(
    measures,
    { currentTheme, activeProfile, activeManifestos, majorOnly, query, locale }
) {
    ...
    if (activeManifestos.length) {
        measures = filter(measures, m =>
            activeManifestos.includes(m.manifestoId)
        )
    }

    // if there's a keyword query active, filter according to that
    measures = filter(measures, m =>
        m.titles[locale].match(new RegExp(query, 'gi'))
    )
...
class ThemeListItem extends Component {
    ...
    render() {
        ...
        return (
            <li
                className={`refinement-list__item ${props.className || ''}`}
                style={state.style}
            >
                {props.children || (
                    <FilterButton
...
class ThemeFilters extends Component {
    ...
    shouldComponentUpdate(props) {
        ...
        if (
            nextActive.length !== activeThemes.length ||
    ...
    filterThemes(themes, items, locale) {
        ...
        let filtered = filter(themes, t =>
            filteredLabels.includes(t.titles[locale])
        )
        ...
        >
            <li className="refinement-list__item refinement-list__item-more">
                <FilterButton onClick={onViewMore}>
                    {T.translate('measures.allthemes', {
                        context: match.params.locale,
                    })}
                </FilterButton>
            </li>
        </ThemeFilters>
    ...
    render() {
        return (
            <Select
                ...
                placeholder={T.translate('browse.filterTheme', {
                    context: this.props.locale,
                })}
                searchable={false}
                clearable={false}
...
class ThemeDetail extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        measures = filterMeasuresForState(measures, {
            currentTheme: theme,
            activeProfile,
            activeManifestos,
            majorOnly,
    ...
    render() {
        ...
        const measuresComponent = this.state.empty ? (
            <NoMeasure theme={theme.title} locale={locale} />
        ) : (
        ...
        return (
            <article className="theme">
                <div style={coverImg} className="theme-image">
                    <div className="theme-image__text">
                        {theme.titles[locale]}
                    </div>
                </div>
