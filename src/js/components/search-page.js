...
class Layout extends Component {
    ...
    syncForTheme(nextProps) {
        ...
        let foundThemes = filter(themes.themes, t =>
            theme.split(',').includes(t.slugs[locale])
        )
    ...
    syncForManifesto(nextProps) {
        ...
        let foundManifestos = filter(manifestos.manifestos, m =>
            manifesto.split(',').includes(m.slugs[locale])
        )

        if (manifesto) {
            foundManifestos.forEach(({ id }) =>
    ...
    shouldComponentUpdate(props, state) {
        if (props.profiles.fetching !== this.props.profiles.fetching) {
            ...
            props.manifestos.fetching !== this.props.manifestos.fetching
        ) {
            return true
        ...
        return (
            ...
            >
                <Sidebar
                    ...
                    manifestos={manifestos.items.map(
                        id => manifestos.manifestos[id]
                    )}
                ...
                <div
                    className={`content ${
                        this.state.showBanner ? 'is-open-banner' : ''
                    }`}
                >
                    <ReactTooltip
                        place="top"
...
export default connect(({ profiles, themes, manifestos, query, locale }) => ({
    ...
    searchState: {
        menu: {
            profileIds: profiles.searchState,
            manifestoIds: manifestos.searchState,
        },
        query: query,
        refinementList: {
            [`titles.${locale}`]: themes.searchState.map(
                id => themes.themes[id].titles[locale]
            ),
        },
    },
}))(Layout)
